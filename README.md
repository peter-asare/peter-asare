#  Peter Boahene Asare
### Geospatial Analyst · BSc Geography & Resource Development · MSc GeoInformation Science

> I turn raw spatial data into decisions — mapping, modelling, and analysing the world one layer at a time.

---

## 🧭 About Me

Geospatial Analyst with a strong academic foundation in Geography and GeoInformation Science.
I work at the intersection of spatial thinking and data analysis — building workflows in GIS
platforms and increasingly leveraging Python to automate and scale geospatial processes.
Always learning, always mapping.

---

## 🛠️ Tools & Technologies

![ArcGIS Pro](https://img.shields.io/badge/ArcGIS%20Pro-2C7BB6?style=flat&logo=esri&logoColor=white)
![ArcMap](https://img.shields.io/badge/ArcMap-2C7BB6?style=flat&logo=esri&logoColor=white)
![Google Earth Engine](https://img.shields.io/badge/Google%20Earth%20Engine-4285F4?style=flat&logo=google&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)

---

## 🎓 Education

| Degree | Field | Level |
|--------|-------|-------|
| MSc | GeoInformation Science | Postgraduate |
| BSc | Geography & Resource Development | Undergraduate |

---

## 🌱 Currently

- Deepening my **Python** skills for GIS automation & spatial analysis
- Exploring geospatial data science workflows with **Google Earth Engine**
- Open to collaborating on **remote sensing**, **environmental mapping**, and **spatial analysis** projects

---

📍 *Let's map something great together.*

<!--
**peter-asare/peter-asare** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
